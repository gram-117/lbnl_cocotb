...
module TotCounter (
    ...
    // ADDED GRAMMY!!!!!
    wire [5:0] stage_mag = init_tot_cnt; 
    logic [3:0] tot_cnt_int;

    always_comb begin
        if (Tot6to4Mapping && (|stage_mag[5:3])) begin
            // keep your intended arithmetic; see note below about 3'
            tot_cnt_int = {1'b1, 3'(stage_mag[5:2] - 4'h2)};
        end else begin
            tot_cnt_int = stage_mag[3:0];
        end
    end
    assign TotOverflow = (tot_cnt_int[3:1] == 3'b111);

    wire phase_bit = cnt_clk; // or ~cnt_clk depending on your intended definition

    wire [3:0] tot_cnt_out = TotDualEdgeCount ? {tot_cnt_int[3:1], phase_bit}
        : tot_cnt_int;

    assign TotCnt = (&(tot_cnt_out[3:1]) ? {tot_cnt_out[3:1], 1'b0} : tot_cnt_out);
